//***    KOLCHCRYPT II - SIMPLY 512-BIT KEY 512-BIT BLOCK CBC-LIKE MODE HASHTRICK ENCRYPTION ALGORITHM       ***//
...
uses
  DCPsha512, DCPHaval,
  Windows, Messages, SysUtils, Variants, Classes, bitmath, Hash;
...
function Kolch_Crypt2 (fi,ft: string; skey: string; dir: byte; process: pprocessproc=nil): boolean;
...
var Hash: TDCP_SHA512;i: integer; Hash2: TDCP_Haval;  BH1: THash_RipeMD256; BH2: THash_Snefru;
begin
Hash:=TDCP_Sha512.Create(nil);
Hash.Init;
Hash.Update(key,64);
...
mutatetables(getbitssum(key[0]));
mutatebittab((getbitssum(key[32])));
mutatenotbittab((getbitssum(key[getbitssum(key[9])])));
